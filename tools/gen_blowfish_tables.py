#!/usr/bin/env python3
"""Generate services/fmo_blowfish_P.bin and fmo_blowfish_S.bin from first
principles.

Front Mission Online's world channel (UDP 61300) uses Blowfish with the
STANDARD pi-derived tables (services/fmoworld.py, which pins that fact in its
selftest). Because the standard tables are simply the fractional hexadecimal
...
can be regenerated from mathematics alone. This script does exactly that:
  ...
  3. serialize little-endian (the layout fmoworld.load_tables reads),
  4. verify the tables against the published Blowfish known-answer test
     (all-zero key, all-zero block -> 4EF997456198DD78) before writing.
...
def main():
    ...
    with open(os.path.join(OUT_DIR, "fmo_blowfish_P.bin"), "wb") as fh:
        fh.write(struct.pack("<18I", *P))
    with open(os.path.join(OUT_DIR, "fmo_blowfish_S.bin"), "wb") as fh:
        fh.write(struct.pack("<1024I", *S))
    print("wrote services/fmo_blowfish_P.bin (72 B) and fmo_blowfish_S.bin (4096 B)")
