...
The shape decided for the private deployment:
  ...
  * ONE SUBDOMAIN PER GAME -- jan.example.com, tm.example.com,
    ...
    deployment's cloudflared maps each hostname to one;
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _forward_bridge(self, rest):
        ...
        loopback. The private deployment gave the bridge tm.example.com rather than a
...
# (services/femap.py), which was the requirement there: the message id
...
class Discord:
    ...
    def _refused(self, what, status, data):
        """A post or edit Discord refused. 400/401/403 are the admin's to
...
# ---------------------------------------------------------------------------
# A board's Discord BOT (decided 2026-09-13): its messages can carry
...
def bot_channels():
    ...
    where an admin told a feed to post (/<board>board <feed>), and where
...
def run_bot_feed(board, args, key, feed, hook, token, opener=None, rounds=None,
    ...
    channel API with the bot's token: into the channel an admin chose
